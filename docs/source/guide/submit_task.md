...
# Quafu（需要 pip install uniq[quafu]）
task_id = submit_task(circuit, backend='quafu', shots=1000, chip_id='ScQ-P10')

# IBM Quantum（需要 pip install uniq[qiskit]）
...
```python
from uniq import list_tasks, get_task, clear_completed_tasks, clear_cache
...
from uniq import submit_task, wait_for_result
...
```python
from uniq import UnifiedResult
...
# 计算期望值
from uniq import calculate_expectation
...
| Quafu | 第三方云平台 | BAQIS ScQ 系列 | `pip install uniq[quafu]` |
| IBM Quantum | 第三方云平台 | IBM Quantum 生态 | `pip install uniq[qiskit]` |
| Dummy | 本地模拟 | 开发测试、联调 | `pip install uniq[simulation]` |
...
- API 参考：
  - {mod}`uniq.task_manager`
  - {mod}`uniq.backend`
  - {mod}`uniq.task.adapters`
  - {mod}`uniq.task.normalizers`
