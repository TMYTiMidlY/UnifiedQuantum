...
当你还没把量子线路表达出来——还不确定如何用 {class}`uniq.circuit_builder.Circuit` 把一个量子算法或实验想法写成可执行的线路表示时，先看本页。

本页解决的核心问题是：**如何从空线路开始，构建一个可以交给模拟器或真机的量子程序**。

## 本页解决的问题 {#guide-circuit-problems}

- 如何创建空线路并添加量子门（{class}`uniq.circuit_builder.Circuit`）
- 如何添加测量指令
...
{func}`uniq.transpiler.draw.draw` 用于绘制线路图：

```python
from uniq.transpiler.draw import draw
...
```python
from uniq.circuit_builder import Parameter
...
```python
from uniq.circuit_builder import Parameters
...
```python
from uniq.circuit_builder import circuit_def, Circuit
