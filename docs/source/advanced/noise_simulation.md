...
当你需要了解如何在模拟中加入噪声模型、模拟真实量子硬件的行为、或配置错误加载器与读取误差时，进入本页。

本页是 [本地模拟](../guide/simulation.md) 的延伸阅读，适合已经完成基础模拟验证、需要更真实模拟环境的读者。

> 如果你还未完成基础线路构建与无噪声模拟，建议先阅读 [本地模拟](../guide/simulation.md)。

UnifiedQuantum 支持在密度矩阵后端上进行带噪声的量子线路模拟。相比理想模拟器，带噪声模拟可以更真实地反映实际量子硬件的行为。
...
from uniq.simulator.error_model import Depolarizing
...
UnifiedQuantum 提供了多种预设错误模型，位于 `uniq.simulator.error_model` 模块中。
...
```python
from uniq.simulator.error_model import BitFlip, Depolarizing
...
```python
from uniq.simulator.error_model import ErrorLoader_GenericError, Depolarizing
...
```python
from uniq.simulator.error_model import ErrorLoader_GateTypeError, Depolarizing, AmplitudeDamping
...
```python
from uniq.simulator.error_model import (
