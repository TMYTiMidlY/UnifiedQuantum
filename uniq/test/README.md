...
# 运行特定类别
pytest uniq/test/core/ -v
pytest uniq/test/simulator/ -v

# 单独运行某个测试
python -m pytest uniq/test/core/test_originir_parser.py::run_test_originir_parser -v

# 通过 run_test() 运行全部（旧方式，已废弃）
from uniq.test import run_test
run_test()
```
...
- **Build-and-test workflow**：使用 `pytest uniq/test/ -v`
- **Pytest Coverage workflow**：使用 `pytest uniq/test/ -v --deselect uniq/test/simulator/test_random_QASM.py::test_random_qasm_compare_density_operator`
