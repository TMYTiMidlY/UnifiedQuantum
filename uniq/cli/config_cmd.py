...
def init():
    ...
    from uniq.config import create_default_config

    create_default_config()
    print_success("Configuration file created at ~/.uniq/uniq.yml")
...
def set(
    ...
    from uniq.config import update_platform_config
...
def get(
    ...
    from uniq.config import get_platform_config
...
def list_config(
    ...
    from uniq.config import PLATFORM_REQUIRED_FIELDS, load_config
...
def profile(
    ...
    from uniq.config import get_active_profile, load_config, set_active_profile
    ...
    if action == "list":
        ...
        if not name:
            print_error("Profile name required")
            raise typer.Exit(1)
        from uniq.config import CONFIG_FILE, load_config, save_config
